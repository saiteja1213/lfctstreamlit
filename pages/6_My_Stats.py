...
# Show actual columns so you can verify
with st.expander("🔍 Debug: Sheet columns (click to verify)"):
    st.write(list(df.columns))

# Rename any common variations → standard internal names
COLUMN_MAP = {
    "match":           "match_name",
...
if missing:
    ...
    st.info(f"**Actual columns in sheet:** {list(df.columns)}")
    st.stop()
...
def render_table(data):
    if data.empty:
        ...
        return
    cols = [c for c in LABEL_MAP.keys() if c in data.columns]
    disp = data[cols].copy()
    disp.columns = [LABEL_MAP[c] for c in cols]
